...
def _gen_object_id(prefix, submitter_id, requested_object_id, coll):
    try:
        ...
        logger.warn(
            msg=f"[_gen_object_id] ? Exception {type(e)} occurred when using {requested_object_id}, using {object_id} instead. message=[{e}] ! traceback={traceback.format_exc()}")
...
async def _remote_submit_file(agent_object_id: str, file_type: str, agent_file_path: str, job_id: str):
    try:
        ...
        logger.info(
            msg=f"{function_name} ({file_type}) connecting to {g_mongo_client_str} anew; agent_object_id:{agent_object_id} file_type:{file_type}, agent_file_path:{agent_file_path} ")
...
async def post_object(parameters: ProviderParameters = Depends(ProviderParameters.as_form),
    ...
    try:
        ...
        logger.info(
            msg=f"[post_object] submitter={parameters.submitter_id}, to service_id={parameters.service_id}, requesting {num_files_requested} files, timeout_seconds={timeout_seconds}")
