...
logger.info(msg=f'redis host={os.getenv("REDIS_HOST")}:{os.getenv("REDIS_PORT")}')
...
def _mongo_insert(coll, obj):
    if mongo_db_major_version < 4:
        logger.info(msg=f"using collection.insert")
        coll.insert(obj)
    else:
        logger.info(msg=f"using collection.insert_one")
        coll.insert_one(obj)


def _mongo_count(coll, obj):
    if mongo_db_major_version < 3 and mongo_db_minor_version < 7:
        logger.info(msg=f"mongodb version = {mongo_db_version}, use deprecated entry count function")
        entry = coll.find(obj, {})
        num_matches = entry[0].count()
    else:
        logger.info(msg=f"mongo_db version = {mongo_db_version}, use count_documents function")
        num_matches = coll.count_documents(obj)
    logger.info(msg=f"found ({num_matches}) matches")
    return num_matches
...
def _get_url(service_id: str, url_type: str = "service_host"):
    ...
    if url_type == "service_host":
        ...
        logger.error("! unrecognized url_type {url_type}")
        return None
...
def _resolveRefs(doc, models):
    if type(doc) == dict:
        if "$ref" in doc:
            ...
            del doc["$ref"]
            logger.info(msg=f"STOP:resolved[name={model_name}, obj={doc[model_name]}]")
            return
        else:
            for k, v in doc.items():
                logger.info(msg=f"resolving dict key:{k}, value:{v}")
                _resolveRefs(doc[k], models)
    elif type(doc) == list:
        for elem in doc:
            logger.info(msg=f"resolving list element {elem}")
            _resolveRefs(elem, models)
    else:
        logger.info(msg=f"STOP:doc type ({type(doc)}) for leaf doc={doc}")
        return
...
async def get_submit_parameters(service_id: str = Query(default="fuse-provider-upload",
    ...
    try:
        ...
        logger.info(msg=f"resolving submit params={params}")
...
def api_add_submitter(submitter_id: str):
    ...
    if num_matches == 1:
        ...
        logger.info(msg=f"submitter_object={submitter_object}")
        _mongo_insert(mongo_submitters, submitter_object.dict())
        logger.info(msg="submitter added.")
    ...
    logger.info(msg=f"returning: {ret_val}")
    return ret_val
...
async def delete_submitter(submitter_id: str = Query(default=None, description="unique identifier for the submitter (e.g., email)")):
    ...
    ret_mongo_err = ""
    try:
        logger.warning(msg=f"Deleting submitter_id:{submitter_id}")
        ...
        if ret.acknowledged != True:
            ...
            logger.error(msg="delete failed, ret.acknowledged ! = True")
        if ret.deleted_count != 1:
            ...
            logger.error(msg=f"delete failed, wrong number deleted, count[1]={ret.deleted_count}")
        ## xxx
        ...
        logger.error(msg=f"Exception {type(e)} occurred while deleting {submitter_id} from database, message=[{e}]")
...
def api_get_submitter(submitter_id):
    ...
    num_matches = _mongo_count(mongo_submitters, {"object_id": object_id})
    logger.info(msg=f"found ({num_matches}) matches for object_id={object_id}")
...
def _gen_object_id(prefix, submitter_id, requested_object_id, coll):
    try:
        ...
        logger.info(msg=f"top prefex={prefix}, submitter={submitter_id}, requested:{requested_object_id}")
        ...
        logger.info(msg=f"found ({num_matches}) matches for requested object_id={requested_object_id}")
        assert num_matches == 0
        return requested_object_id
    except Exception as e:
        logger.warning(
            msg=f"? Exception {type(e)} occurred when using {requested_object_id}, using {object_id} instead. message=[{e}] ! traceback={traceback.format_exc()}")
        logger.warning(msg=f"[_gen_object_id] ")
        return object_id
...
async def _remote_submit_file(agent_object_id: str, file_type: str, agent_file_path: str):
    try:
        ...
        logger.info(
            msg=f"({file_type}) connecting to {mongo_client_str} anew; agent_object_id:{agent_object_id} file_type:{file_type}, agent_file_path:{agent_file_path} ")
        ...
        logger.info(msg=f"({file_type}) host_url={host_url}")
        submit_url = f"{host_url}/submit"
        logger.info(msg=f"({file_type}) posting to url={submit_url}")
        # common code ^^^
        #####
        (agent_file_dir, agent_file_name) = os.path.split(agent_file_path)
        logger.info(msg=f"({file_type}) posting file {agent_file_name} from directory {agent_file_path}, type {file_type}")
        ...
        logger.info(msg=f"({file_type}) provider request complete for this file, removing file {agent_file_path}")
        ...
        if response.status_code == 200:
            ...
            logger.info(msg=f"({file_type}) set file_type={file_type} = {service_object_id}; loaded_file_objects={loaded_file_objects}")
            m_objects.update_one({"object_id": agent_object_id},
                                 {"$set": {
            ...
            logger.error(msg=f"({file_type}) ! {detail_str}")
            m_objects.update_one({"object_id": agent_object_id},
                                 {"$set": {
                                     "agent_status": "failed",
        ...
        logger.info(msg=f"({file_type}) object {agent_object_id} successfully created.")
        try:
            ...
            assert _mongo_count(m_objects, {"object_id": agent_object_id}) == 1
            obj = entry[0]
            logger.info(msg=f'({file_type}) obj={obj}')
            logger.info(msg=f'({file_type}) Loaded objects={len(obj["loaded_file_objects"])}, num requested={obj["num_files_requested"]}')
            if len(loaded_file_objects) == obj["num_files_requested"]:
                logger.info(msg=f"({file_type}) Removing directory {agent_file_dir}")
                os.rmdir(agent_file_dir)
                m_objects.update_one({"object_id": agent_object_id},
                                     {"$set": {
                                         "agent_status": "finished"
                                     }})
                logger.info(msg=f"({file_type}) ({agent_object_id}) agent_status = finished")
        except Exception as e:
            logger.error(
                msg=f'({file_type}) ! Exception {type(e)} occurred while attempting to unlink file {agent_file_dir} for object {agent_object_id}, message=[{e}] ! traceback={traceback.format_exc()}')


    except Exception as e:
        detail_str += f"! Exception {type(e)} occurred while submitting object to service, message=[{e}] ! traceback={traceback.format_exc()}"
        logger.error(msg=f"({file_type}) ! status=failed, {detail_str}")
        try:
            ...
            logger.error(msg=f'({file_type}) ! unable to update object to failed.')
        logger.error(msg=f'({file_type}) ! updated object {agent_object_id} to failed.')


async def _immunespace_submit(agent_object_id: str, submitter_id: str, accession_id: str, apikey: str):
    try:
        logger.info(msg=f"looking up {agent_object_id}")

        local_mongo_client = pymongo.MongoClient(mongo_client_str)
        local_mongo_db = local_mongo_client.test
        local_mongo_objects = local_mongo_db.objects

        entry = local_mongo_objects.find({"object_id": agent_object_id}, {"_id": 0})
        assert _mongo_count(local_mongo_objects, {"object_id": agent_object_id}) == 1
        found_object = entry.next()
        logger.info(msg=f"found_object: {found_object}")
        host_url = _get_url(found_object["parameters"]["service_id"])

        local_mongo_objects.update_one({"object_id": agent_object_id}, {"$set": {"service_host_url": host_url, "agent_status": "started"}})

        logger.info(msg=f"host_url={host_url}")
        submit_url = f"{host_url}/submit"
        logger.info(msg=f"posting to url={submit_url}")

        params = {"submitter_id": submitter_id, "accession_id": accession_id, "apikey": apikey}
        response = requests.post(submit_url, params=params)

        if response.status_code == 200:
            response_json = response.json()
            service_object_id = response_json["object_id"]
            logger.info(msg=f"service_object_id: {service_object_id}")
            detail_str = f'status_code={response.status_code}, response={response.text}'
            local_mongo_objects.update_one({"object_id": agent_object_id},
                                           {"$set": {"agent_status": "finished", "service_object_id": service_object_id, "detail": f'{detail_str}'}})
        else:
            detail_str = f'status_code={response.status_code}, response={response.text}'
            local_mongo_objects.update_one({"object_id": agent_object_id}, {"$set": {"agent_status": "failed", "detail": f'{detail_str}'}})

        entry = local_mongo_objects.find_one({"object_id": agent_object_id})
        logger.info(msg=f"entry: {entry}")

        logger.info(msg=f"object {agent_object_id} successfully created.")
    except Exception as e:
        logger.error(msg=f'! updated object {agent_object_id} to failed. message=[{e}], traceback: {traceback.format_exc()}')
...
async def post_object(parameters: ProviderParameters = Depends(ProviderParameters.as_form),
    ...
    if parameters.submitter_id is not None and parameters.accession_id is not None and parameters.apikey is not None:
        # we have an immunespace load/submit request

        agent_object_id = _gen_object_id("agent", parameters.submitter_id, requested_object_id, mongo_objects)
        logger.info(msg=f"agent_object_id: {agent_object_id}")
        ...
        logger.info(msg=f"inserting agent-side provider_object={provider_object}")
        _mongo_insert(mongo_objects, provider_object)
        logger.info(msg=f"created provider object: object_id:{agent_object_id}, submitter_id:{parameters.submitter_id}")

        job_id = str(uuid.uuid4())
        g_queue.enqueue(_immunespace_submit, agent_object_id=agent_object_id, submitter_id=parameters.submitter_id, accession_id=parameters.accession_id, apikey=parameters.apikey,
        ...
        # we have an upload load/submit request
        ...
        try:
            ...
            logger.info(msg=f"getting id")
            agent_object_id = _gen_object_id("agent", parameters.submitter_id, requested_object_id, mongo_objects)
            timeout_seconds = g_redis_default_timeout  # read this from config.json for the service xxx
            logger.info(
                msg=f"submitter={parameters.submitter_id}, to service_id={parameters.service_id}, requesting {num_files_requested} files, timeout_seconds={timeout_seconds}")
            ...
            os.mkdir(agent_path)
            logger.info(msg=f"upload file path = {agent_path}")

            if parameters.accession_id is not None:
                logger.info(msg=f"calling service with accession id = {parameters.accession_id, parameters.apikey}")

            for file_type in client_file_dict.keys():
                client_file_obj = client_file_dict[file_type]
                if client_file_obj is not None:
                    client_file_name = client_file_obj.filename
                    logger.info(msg=f"getting file = {client_file_name}, file_type= {file_type}")  # ok so far
                    ...
                    import time
                    logger.info(msg=f"sleep a sec to try and avoid racing conditions")
                    time.sleep(3)
                    ...
                    logger.info(msg=f"QUEUE: agent_object_id:{agent_object_id}, file_type:{file_type}, agent_file_path:{agent_file_path},job_id=:{job_id}")  # ok so far
                    g_queue.enqueue(_remote_submit_file, args=(agent_object_id, file_type, agent_file_path), timeout=timeout_seconds, job_id=job_id, result_ttl=-1)
            ...
            try:
                ...
                logger.error(msg=f"! unable to change agent_object_id to agent_status=failed")
            logger.error(msg=f"! {detail_str}")
...
async def get_url(object_id: str, file_type: str):
    ...
    try:
        ...
        logger.info(msg=f"find local object={object_id}")
        entry = mongo_objects.find({"object_id": object_id}, {"_id": 0})
        assert _mongo_count(mongo_objects, {"object_id": object_id}) == 1
        obj = entry.next()
        logger.info(msg=f'found local object, agent_status={obj["agent_status"]}')
        logger.info(msg=f'obj={obj}')
        ...
        logger.info(msg=f"built url = ={obj_url}")
...
def _remote_delete_object(agent_object_id: str):
    ...
    try:
        ...
        logger.info(msg=f"connecting to {mongo_client_str} anew")
        my_mongo_client = pymongo.MongoClient(mongo_client_str)
        my_mongo_db = my_mongo_client.test
        m_objects = my_mongo_db.objects
        ...
        logger.info(msg=f"delete remote object with {delete_url}")
        ...
        if response.status_code == 404:
            logger.warning(msg=f"Remote object {service_object_id} not found with {delete_url} when deleting {agent_object_id}")
            # xxx may want to add this to an audit report for admin or something

        logger.warning(msg=f"Deleting agent agent_object_id: {agent_object_id}")
        ...
        if ret.acknowledged != True:
            ...
            logger.error(msg=f"agent delete failed, ret.acknowledged ! = True")
        if ret.deleted_count != 1:
            # should never happen if index was created for this field
            delete_status = "failed"
        ...
        logger.error(
            msg=f"Exception {type(e)} occurred while deleting agent {agent_object_id} from database, message=[{e}]  ! traceback={traceback.format_exc()}")
    ...
    logger.info(msg=f"Deleting agent {agent_object_id} from file system")
    ret_os = ""
    ret_os_err = ""
    try:
        ...
        logger.info(msg=f"removing tree ({local_path})")
        shutil.rmtree(local_path, ignore_errors=False)
    except Exception as e:
        logger.warn(msg=f"Exception {type(e)} occurred while deleting agent {agent_object_id} from filesystem")
...
async def delete(object_id: str):
    ...
    try:
        ...
        logger.info(msg=f"deleting {object_id}")
        ret = _remote_delete_object(object_id)
        logger.info(msg=f"returned ({ret})")
        info = ret["info"]
        ...
        logger.error(msg=f"Exception {type(e)} occurred while deleting agent {object_id} from filesystem. detail_str={detail_str}")
        raise HTTPException(status_code=404,
                            detail=detail_str)
...
async def get_object(object_id: str = Query(default=None, description="unique identifier on agent to retrieve previously loaded object")):
    ...
    try:
        ...
        logger.info(msg=f"Finding metadata for object {object_id}")
        entry = mongo_objects.find({"object_id": object_id}, {"_id": 0})
        assert _mongo_count(mongo_objects, {"object_id": object_id}) == 1
        obj = entry[0]
        logger.info(msg=f'found local object, agent_status={obj["agent_status"]}')
        ...
        if obj["agent_status"] == "finished":
            for file_type in obj["loaded_file_objects"]:
                ...
                logger.info(msg=f'({file_type}) REQUEST: {service_object_id}')
                response = requests.get(f'{obj["service_host_url"]}/objects/{service_object_id}')
                service_obj_metadata = response.json()
                logger.info(msg=f'({file_type}) METADATA={service_obj_metadata}')
...
async def _remote_analyze_object(agent_object_id: str, parameters: ToolParameters):
    try:
        ...
        service_object_id = None  # set this early in case there's an exception
        # get the agent object for this job
        logger.info(msg=f"({file_type}) looking up {agent_object_id}")
        entry = m_objects.find({"object_id": agent_object_id}, {"_id": 0})
        assert _mongo_count(m_objects, {"object_id": agent_object_id}) == 1
        obj = entry[0]
        ...
        try:
            ...
            logger.error(msg=f'({agent_object_id}) ! unable to update object to failed.')
        logger.error(msg=f'! updated object {agent_object_id} to failed.')
...
4. If the meta data shows status = finished, the dashboard uses the link in the meta data to retrieve the results. (edited) 
    ...
    try:
        ...
        logger.info("if record for this submitter ({parameters.submitter_id}) is not found, create one")
        add_submitter_response = api_add_submitter(parameters.submitter_id)
        ...
        logger.info(msg=f"inserting agent-side agent_object={agent_object}")
        _mongo_insert(mongo_objects, agent_object)
        logger.info(msg=f"created agent object: object_id:{agent_object_id}, submitter_id:{parameters.submitter_id}")
        # END
        #####
        ...
        # enqueue the job
        job_id = str(uuid.uuid4())
        logger.info(msg=f"submitter={parameters.submitter_id}, to service_id={parameters.service_id}, timeout_seconds={timeout_seconds}, job_id={job_id}")
        ...
        raise HTTPException(status_code=404,
                            detail="! Exception {type(e)} occurred while running submit, message=[{e}] ! traceback={traceback.format_exc()}")
